...
import 'package:klubba/view/full_image.dart';
...
class SelectCoachScreen extends StatefulWidget {
  final String coachSlug;

  SelectCoachScreen(this.coachSlug);

  CoachState createState() => CoachState();
}

class CoachState extends State<SelectCoachScreen> {
  ...
  Map<String,dynamic> coachResults={};
    ...
    else if(!ratingIcon1 && !ratingIcon2 && !ratingIcon3 && !ratingIcon4 && !ratingIcon5 && !ratingIcon6)
{
  ...
  fetchCurrentCat() async {
     catID = await MyUtils.getSharedPreferences('current_category_id');
  ...
  coachDetails(BuildContext context) async {
    ...
    var response =
        await helper.postAPI('coachDetailForHomePage', requestModel, context);
    ...
    if (responseJSON['decodedData']['status'] == 'success') {
      ...
      coachName = responseJSON['decodedData']['coach_user']['full_name'];
      coachResults = responseJSON['decodedData']['result'];

      aboutCoach = responseJSON['decodedData']['result']['about'];
      mapLocation = responseJSON['decodedData']['result']['map_location'];
      target = responseJSON['decodedData']['result']['_id'];
      targetName = responseJSON['decodedData']['result']['profession_name'];
      isKlubbaVerified =
      responseJSON['decodedData']['coach_user']['is_klubba_verified'];
  ...
  GestureDetector slider(images, pagePosition, active) {
    double margin = active ? 8 : 8;

    return GestureDetector(
      onTap: (){
        Navigator.push(context, MaterialPageRoute(builder: (context) =>
            FullPhotoScreen(images, pagePosition, images[pagePosition])));
      },
  ...
  saveRating(BuildContext context) async {
    ...
    var data = {
      ...
      "data":{
        "rate_user_slug":widget.coachSlug,
        "rating":5,
  ...
  reportUser(BuildContext context,String subject,String message) async {
    ...
    "target":coachResults['_id'],
    "target_type":"coach",
    "target_name":coachResults['profession_name'],
    "target_link":"",
    "subject":subject,
    "enquiry_type":"report",
    "first_name":name,
